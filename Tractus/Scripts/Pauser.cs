...
public class Pauser : MonoBehaviour {
	...
	void OnPause()
	{
		...
		pauseBehavs = Array.FindAll(GetComponentsInChildren<Behaviour>(), (obj) => obj != null && obj.enabled);
		foreach (var com in pauseBehavs)
		{
			com.enabled = false;
		}

		rgBodies = Array.FindAll(GetComponentsInChildren<Rigidbody>(), (obj) => obj != null && !obj.IsSleeping());
		rgBodyVels = new Vector3[rgBodies.Length];
		rgBodyAVels = new Vector3[rgBodies.Length];
		for (var i = 0; i < rgBodies.Length; ++i)
		{
			rgBodyVels[i] = rgBodies[i].linearVelocity;
			rgBodyAVels[i] = rgBodies[i].angularVelocity;
			rgBodies[i].Sleep();
		}

		rg2dBodies = Array.FindAll(GetComponentsInChildren<Rigidbody2D>(), (obj) => obj != null && !obj.IsSleeping());
		rg2dBodyVels = new Vector2[rg2dBodies.Length];
		rg2dBodyAVels = new float[rg2dBodies.Length];
		for (var i = 0; i < rg2dBodies.Length; ++i)
		{
			rg2dBodyVels[i] = rg2dBodies[i].linearVelocity;
	...
	void OnResume() {
		...
		for ( var i = 0 ; i < rgBodies.Length ; ++i ) {
			rgBodies[i].WakeUp();
			rgBodies[i].linearVelocity = rgBodyVels[i];
			rgBodies[i].angularVelocity = rgBodyAVels[i];
		}

		for ( var i = 0 ; i < rg2dBodies.Length ; ++i ) {
			rg2dBodies[i].WakeUp();
			rg2dBodies[i].linearVelocity = rg2dBodyVels[i];
